...
class ColorMapCreator:
    ...
    def __init__(self, RGB1, RGB2, numColors=33., divide=255., 
                  method="moreland", filename="", refPoint=None):

        if refPoint is None:
            # from Adobe Cookbook
            refPoint = [95.047, 100.0, 108.883]
        # Reference white-point D65
        self.Xn, self.Yn, self.Zn = refPoint 
        ...
        if method == knownMethods[0]:
            ...
            colorMap = self.generateColorMap(RGB1, RGB2, divide)
...
def main():
    ...
    N = 2001
    # EAS modified endpoints
    #RGB1 = np.array([30, 38, 90])
    #RGB2 = np.array([200, 6, 40])
    #refPoint = [159.0, 107.0, 95.0]
    ...
    fname = "colormap.csv"
    filename = os.path.join(current_directory, fname)
    colormap = ColorMapCreator(
        RGB1, RGB2, N, divide=1, filename=filename, refPoint=refPoint)
